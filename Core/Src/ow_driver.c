...
OW_ERROR_E Search(Sensor_t* sensors[], uint8_t size)
{
	...
	do{
		...
		if (last_discrepancy)
		{
			search_frame[5 + last_discrepancy / 4] |= 1 << (2 * (last_discrepancy % 4) + 1);
...
static OW_ERROR_E SelectSensor(Sensor_t* sensor)
{
	...
	Command_t rom_code_command =
	{
		.request_frame = &sensor->addr,
		.request_size = 8,
		.responce_size = 8
	};
...
static OW_ERROR_E RunCommand(Command_t* command, uint16_t* ptr)
{
	...
	*ptr = start_write_ptr = write_ptr;
	...
	HAL_UART_Transmit(uart, command->request_frame, command->request_size, HAL_MAX_DELAY);
...
void ReceiveUartTask(void const *argument)
{
	...
	while(1){
		...
		if (evt.status == osEventMessage) {
			...
			if (current_command)
			{
				uint16_t len = LEN(start_write_ptr, write_ptr) - 1;
